...
主线 BitTorrent 客户端使用与 μTorrent 相同的代码，这个工具对它也有效果。
...
- 拒绝上传，由于各种原因导致连接的对端始终不传输哪怕 1 字节有效数据，如恶意吸血、
  网络不畅、μTorrent 自身 bug 或设置不合理等。在检测到拒绝标记后的一段时间后，
...
```
ban_peers -h
欢迎使用 Ban-Peers 1.0.6

用法: ban_peers [-H IP|域名] [-p 端口] [-a 用户名:密码] [-e 小时] [-t 分钟]
                [-f 格式] [-C] [-X] [-P] [-L] [-N] [-R] [-M] [-U] [-A] [-O]
                [-s [配置文件] | -l [配置文件]] [-h] [-v]
                [IP屏蔽配置路径]
...
- 网页 API 返回的对端数据中，数值被转换成 32 位有符号数，这会导致数值溢出。2G
  ...
  而误报。
- 经典桌面版的 μTorrent 似乎会将其移动版本识别为伪造客户端，Ban-Peers 不会对此
  采取特别措施，统一按伪造处理，如有困扰请直接向官方回报反馈。
